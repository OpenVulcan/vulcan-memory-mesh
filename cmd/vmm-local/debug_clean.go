...
func parseDebugCleanSelection(raw string) (debugCleanSelection, error) {
	...
	if len(parts) == 0 {
		return selection, fmt.Errorf("debug-clean target is required: use duckdb, lancedb, or all")
	}
	for _, part := range parts {
		...
		case "":
			continue
		default:
			return debugCleanSelection{}, fmt.Errorf("unsupported debug-clean target %q: use duckdb, lancedb, or all", part)
		}
	}
	if !selection.DuckDB && !selection.LanceDB {
		return debugCleanSelection{}, fmt.Errorf("debug-clean target is required: use duckdb, lancedb, or all")
	}
