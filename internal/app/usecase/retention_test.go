...
type fakeRetentionStore struct {
	...
	purgeResult   logicdomain.MemoryTrashPurgeResult
}
...
// PurgeExpiredMemoryTrash records the latest purge inputs and returns the configured fake result.
// PurgeExpiredMemoryTrash 用于记录最近一次 purge 输入，并返回预设的 fake 结果。
func (f *fakeRetentionStore) PurgeExpiredMemoryTrash(_ context.Context, before time.Time, limit int) (logicdomain.MemoryTrashPurgeResult, error) {
...
// TestRetentionUseCaseRunMaintenanceRecyclesAndPurges verifies one maintenance pass forwards the normalized protection floors, bridges vector cleanup, and computes the trash purge threshold from config.
// TestRetentionUseCaseRunMaintenanceRecyclesAndPurges 用于验证一次维护会透传规范化后的保护阈值、衔接向量清理，并根据配置计算回收站 purge 阈值。
func TestRetentionUseCaseRunMaintenanceRecyclesAndPurges(t *testing.T) {
	...
	if len(vector.deletedIDs) != 2 || vector.deletedIDs[0] != "vec-1" || vector.deletedIDs[1] != "vec-2" {
