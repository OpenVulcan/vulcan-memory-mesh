// cstring_test.go verifies the bounded SQLite FFI C-string reader keeps normal strings intact and rejects malformed unterminated buffers.
// cstring_test.go 用于验证带上界的 SQLite FFI C 字符串读取器既能保留正常字符串，也会拒绝畸形的非终止缓冲区。
package sqliteffi
