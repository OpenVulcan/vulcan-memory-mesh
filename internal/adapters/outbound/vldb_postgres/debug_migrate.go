...
func (s *Store) importManagedSnapshot(ctx context.Context, snapshot storagemigrate.Snapshot) (storagemigrate.Report, error) {
	...
	if err := s.truncateManagedTables(callCtx, tx); err != nil {
		return storagemigrate.Report{}, err
	}
	if err := s.importNoiseEmbeddings(callCtx, tx, snapshot.NoiseEmbeddings); err != nil {
		return storagemigrate.Report{}, err
	}
	if err := s.importUsers(callCtx, tx, snapshot.Users); err != nil {
		return storagemigrate.Report{}, err
	}
	if err := s.importTeams(callCtx, tx, snapshot.Teams); err != nil {
		return storagemigrate.Report{}, err
	}
	if err := s.importSpaces(callCtx, tx, snapshot.Spaces); err != nil {
		return storagemigrate.Report{}, err
	}
	if err := s.importProjects(callCtx, tx, snapshot.Projects); err != nil {
		return storagemigrate.Report{}, err
	}
	if err := s.importSessions(callCtx, tx, snapshot.Sessions); err != nil {
		return storagemigrate.Report{}, err
	}
	if err := s.importTurns(callCtx, tx, snapshot.Turns); err != nil {
		return storagemigrate.Report{}, err
	}
	if err := s.importMemoryNodes(callCtx, tx, snapshot.MemoryNodes); err != nil {
		return storagemigrate.Report{}, err
	}
	if err := s.importMemoryContextEdges(callCtx, tx, snapshot.MemoryContextEdges); err != nil {
		return storagemigrate.Report{}, err
	}
	if err := s.importProfileNodes(callCtx, tx, snapshot.ProfileNodes); err != nil {
		return storagemigrate.Report{}, err
	}
	if err := s.importProfileInstructions(callCtx, tx, snapshot.ProfileInstructions); err != nil {
		return storagemigrate.Report{}, err
	}
	...
	if err := s.persistManagedSnapshotVersions(callCtx); err != nil {
		return storagemigrate.Report{}, err
	}
...
func (s *Store) truncateManagedTables(ctx context.Context, tx pgx.Tx) error {
...
`, s.profileInstructionsTable(), s.profileNodesTable(), s.memoryContextEdgesTable(), s.memoryNodesTable(), s.turnsTable(), s.sessionsTable(), s.projectsTable(), s.spacesTable(), s.teamsTable(), s.usersTable(), s.noiseEmbeddingsTable(), s.schemaTable())
...
func (s *Store) persistManagedSnapshotVersions(ctx context.Context) error {
	return s.writeTrackedSchemaVersions(ctx)
}
...
func (s *Store) importNoiseEmbeddings(ctx context.Context, tx pgx.Tx, entries []logicdomain.NoiseEmbeddingCacheEntry) error {
	statement := fmt.Sprintf(`
...
`, s.noiseEmbeddingsTable())
	return queueBatches(ctx, tx, s.cfg.MigrationBatchSize, len(entries), func(batch *pgx.Batch, start, end int) error {
		for _, entry := range entries[start:end] {
			if err := validateMigrationVectorDimension("noise embedding", entry.Phrase, entry.Vector, s.cfg.EmbeddingDimension); err != nil {
				return err
			}
...
func (s *Store) importUsers(ctx context.Context, tx pgx.Tx, users []logicdomain.UserRecord) error {
	statement := fmt.Sprintf(`
INSERT INTO %s (id, name, profile, delete_confirm_code, created_at, updated_at)
VALUES ($1, $2, $3, $4, $5, $6)
`, s.usersTable())
	return queueBatches(ctx, tx, s.cfg.MigrationBatchSize, len(users), func(batch *pgx.Batch, start, end int) error {
...
func (s *Store) importTeams(ctx context.Context, tx pgx.Tx, teams []logicdomain.TeamRecord) error {
	statement := fmt.Sprintf(`
INSERT INTO %s (id, name, profile, created_at, updated_at)
VALUES ($1, $2, $3, $4, $5)
`, s.teamsTable())
	return queueBatches(ctx, tx, s.cfg.MigrationBatchSize, len(teams), func(batch *pgx.Batch, start, end int) error {
...
func (s *Store) importSpaces(ctx context.Context, tx pgx.Tx, spaces []logicdomain.SpaceRecord) error {
	statement := fmt.Sprintf(`
INSERT INTO %s (id, team_id, name, profile, created_at, updated_at)
VALUES ($1, $2, $3, $4, $5, $6)
`, s.spacesTable())
	return queueBatches(ctx, tx, s.cfg.MigrationBatchSize, len(spaces), func(batch *pgx.Batch, start, end int) error {
...
func (s *Store) importProjects(ctx context.Context, tx pgx.Tx, projects []logicdomain.ProjectRecord) error {
	statement := fmt.Sprintf(`
INSERT INTO %s (id, team_id, space_id, name, profile, created_at, updated_at)
VALUES ($1, $2, $3, $4, $5, $6, $7)
`, s.projectsTable())
	return queueBatches(ctx, tx, s.cfg.MigrationBatchSize, len(projects), func(batch *pgx.Batch, start, end int) error {
...
func (s *Store) importSessions(ctx context.Context, tx pgx.Tx, sessions []logicdomain.SessionRecord) error {
	statement := fmt.Sprintf(`
INSERT INTO %s (
...
`, s.sessionsTable())
	return queueBatches(ctx, tx, s.cfg.MigrationBatchSize, len(sessions), func(batch *pgx.Batch, start, end int) error {
...
func (s *Store) importTurns(ctx context.Context, tx pgx.Tx, turns []logicdomain.SessionTurnRecord) error {
	statement := fmt.Sprintf(`
INSERT INTO %s (
...
`, s.turnsTable())
	return queueBatches(ctx, tx, s.cfg.MigrationBatchSize, len(turns), func(batch *pgx.Batch, start, end int) error {
...
func (s *Store) importMemoryNodes(ctx context.Context, tx pgx.Tx, rows []logicdomain.MemoryNodeRecord) error {
	statement := fmt.Sprintf(`
INSERT INTO %s (
...
`, s.memoryNodesTable())
	return queueBatches(ctx, tx, s.cfg.MigrationBatchSize, len(rows), func(batch *pgx.Batch, start, end int) error {
		for _, row := range rows[start:end] {
			if err := validateMigrationVectorDimension("memory node", row.VectorID, row.Vector, s.cfg.EmbeddingDimension); err != nil {
				return err
			}
			batch.Queue(statement,
				int64(row.ID),
...
func (s *Store) importMemoryContextEdges(ctx context.Context, tx pgx.Tx, rows []logicdomain.MemoryContextEdge) error {
	statement := fmt.Sprintf(`
INSERT INTO %s (
...
func (s *Store) importProfileNodes(ctx context.Context, tx pgx.Tx, rows []logicdomain.ProfileNodeRecord) error {
	statement := fmt.Sprintf(`
INSERT INTO %s (
...
func (s *Store) importProfileInstructions(ctx context.Context, tx pgx.Tx, rows []logicdomain.ProfileInstructionRecord) error {
	statement := fmt.Sprintf(`
INSERT INTO %s (
