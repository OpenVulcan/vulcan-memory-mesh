// workspace_timeout_test.go verifies project-listing timeout selection so online paths keep their normal query budget while maintenance flows can opt into longer reads.
// workspace_timeout_test.go 用于验证项目列表枚举的超时选择，确保在线路径继续使用常规查询预算，而维护流程可以显式切到更长读取预算。
...
func TestListProjectsWithQueryerUsesOnlineQueryTimeout(t *testing.T) {
	...
	if len(projects) != 0 {
		t.Fatalf("expected no rows, got %+v", projects)
	}
...
// captureProjectListQueryer records the query context chosen by the project-listing helper so timeout-selection tests can assert which budget was applied without a live database.
// captureProjectListQueryer 用于记录项目枚举辅助逻辑选中的查询上下文，让超时测试无需真实数据库也能断言具体套用了哪一类预算。
type captureProjectListQueryer struct {
...
func (*captureProjectListQueryer) Exec(context.Context, string, ...any) (pgconn.CommandTag, error) {
...
func (q *captureProjectListQueryer) Query(ctx context.Context, sql string, args ...any) (pgx.Rows, error) {
	q.lastCtx = ctx
	q.lastSQL = sql
	q.lastArgs = append([]any(nil), args...)
	return emptyProjectRows{}, nil
}

// QueryRow should stay unused in these timeout-selection tests.
// QueryRow 在这些超时选择测试中不应被调用。
func (*captureProjectListQueryer) QueryRow(context.Context, string, ...any) pgx.Row {
	return unexpectedProjectListRow{}
}

// unexpectedProjectListRow fails immediately when a QueryRow-based path is accidentally used in these tests.
// unexpectedProjectListRow 用于在测试意外走到 QueryRow 路径时立即失败。
type unexpectedProjectListRow struct{}

// Scan always reports misuse because QueryRow is not part of the project-listing path under test.
// Scan 会始终报错，因为当前测试覆盖的项目枚举路径不应依赖 QueryRow。
func (unexpectedProjectListRow) Scan(...any) error {
	return fmt.Errorf("unexpected QueryRow scan")
}

// emptyProjectRows is the minimal pgx.Rows stub needed by timeout-selection tests that only care about the context used to start the query.
// emptyProjectRows 用于提供超时选择测试所需的最小 pgx.Rows 存根；这些测试只关心查询启动时使用的上下文。
type emptyProjectRows struct{}

// Close is a no-op because the stub never owns external resources.
// Close 为空操作，因为该存根不持有外部资源。
func (emptyProjectRows) Close() {}

// Err reports no row-iteration failure.
// Err 用于报告“没有行迭代错误”。
func (emptyProjectRows) Err() error { return nil }

// CommandTag returns one zero tag because these tests never inspect row-count metadata.
// CommandTag 用于返回零值标签，因为这些测试不会检查行数元数据。
func (emptyProjectRows) CommandTag() pgconn.CommandTag { return pgconn.CommandTag{} }

// FieldDescriptions returns nil because the stub never exposes actual columns.
// FieldDescriptions 返回 nil，因为该存根不会暴露真实列定义。
func (emptyProjectRows) FieldDescriptions() []pgconn.FieldDescription { return nil }

// Next always reports the empty result set immediately.
// Next 会立刻报告空结果集。
func (emptyProjectRows) Next() bool { return false }

// Scan fails because the empty-row stub should never be scanned.
// Scan 会报错，因为空结果集存根不应进入扫描阶段。
func (emptyProjectRows) Scan(...any) error {
