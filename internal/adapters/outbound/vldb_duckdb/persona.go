...
func (s *Store) Load(ctx context.Context, session logicdomain.SessionRef) (logicdomain.PersonaContext, error) {
	// Keep the provider contract satisfied with a stable no-op result until DuckDB-backed persona records are introduced.
	// 在 DuckDB 画像记录尚未落地前，先用稳定的空结果满足 provider 契约，避免重新引入内存回退。
	select {
	case <-ctx.Done():
