...
// LoadActiveSessionMemoryNodes returns the active memory-node anchors inside one session so the single-turn analyzer can decide which old memories to supersede.
// LoadActiveSessionMemoryNodes 用于返回某个 session 内的活跃记忆节点锚点，让单轮分析器判断哪些旧记忆需要淘汰。
