...
// TestNewClientUsesSharedBoundedTransport verifies default SiliconFlow instances cannot multiply connections or regain an HTTP-wide request timeout.
// TestNewClientUsesSharedBoundedTransport 用于验证默认 SiliconFlow 实例不会放大连接数量或重新获得 HTTP 全局请求超时。
func TestNewClientUsesSharedBoundedTransport(t *testing.T) {
	...
	if client.httpClient != httpclient.SharedDefault() {
		t.Fatal("SiliconFlow rerank must reuse the process-wide bounded HTTP client")
	}
	if client.httpClient.Timeout != 0 {
		t.Fatalf("SiliconFlow shared HTTP timeout = %s, want 0", client.httpClient.Timeout)
	}
	if client.timeout != defaultTimeout {
		t.Fatalf("SiliconFlow operation timeout = %s, want %s", client.timeout, defaultTimeout)
	}
	if defaultTimeout != 8*time.Second {
		t.Fatalf("SiliconFlow default timeout = %s, want 8s", defaultTimeout)
	}
}
