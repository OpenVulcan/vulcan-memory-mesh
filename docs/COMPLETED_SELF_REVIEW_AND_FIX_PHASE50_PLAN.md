# SELF REVIEW AND FIX PHASE50 PLAN

## 任务目标

- 在当前干净基线下继续执行一轮真实自检。
- 优先审阅 usecase 层导出入口与直接调用路径，确认是否仍存在可被直接触发的不完善点或风险问题。
...
- 本轮确认了一个真实问题：`PreCheckUseCase.Execute(...)` 内部大量依赖已经按“缺失则降级”设计，但导出入口本身没有 `nil receiver` 防护。
- 该问题会导致直接测试或手工集成误把 `Execute(...)` 调到 `nil *PreCheckUseCase` 上时，在访问 `u.loadPersonaContext(...)` 之前就触发真实 panic，而不是返回稳定错误。
- 已完成修复：
  - `internal/app/usecase/precheck.go`
  - 在 `Execute(...)` 开头新增 `nil receiver` 的快速失败保护。
- 已补回归测试：
  - `TestPreCheckExecuteRejectsNilReceiver`
- 已完成验证：
