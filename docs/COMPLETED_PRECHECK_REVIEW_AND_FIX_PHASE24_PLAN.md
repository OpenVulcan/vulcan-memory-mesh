# PRECHECK REVIEW AND FIX PHASE 24 PLAN

## 任务目标

- 继续审阅 `PreCheck` 候选构造、第二层评审恢复与上下文组装链。
- 找出一个真实且可复现的问题，并完成修复、测试补强与闭环验证。
- 保持现有对外 gRPC 契约不变，只修正内部实现的一致性、稳定性或副作用问题。

## 执行步骤

1. 审阅 `searchMemoryCandidates(...)`、`reviewMemoryCandidates(...)`、上下文组装和相关测试。
2. 确认是否存在候选输出、reviewer 恢复、score 展示或上下文注入上的真实不一致问题。
