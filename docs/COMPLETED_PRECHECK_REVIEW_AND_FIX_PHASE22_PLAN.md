# PRECHECK REVIEW AND FIX PHASE 22 PLAN

## 任务目标

- 继续审阅 `PreCheck` 候选筛选、第二层评审恢复以及 fallback/最终注入链。
- 找出一个真实且可复现的问题，并完成修复、测试补强与闭环验证。
- 保持现有对外 gRPC 契约不变，仅修正内部实现的一致性、稳定性或副作用问题。

## 执行步骤

1. 审阅 `searchMemoryCandidates(...)`、`reviewMemoryCandidates(...)`、fallback 渲染与最终注入相关实现及测试。
2. 确认是否存在阈值筛选、重复恢复、fallback 显示、写回副作用或最终输出一致性上的真实问题。
