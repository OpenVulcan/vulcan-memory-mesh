...
8. “一个领域”不等于“一个名词一条节点”：
   ...
   - 例如“喜欢苹果和香蕉”可以合并为一条饮食偏好节点
   - 例如“喜欢饮茶和饮料”可以合并为一条饮品偏好节点
   - 不要把同领域的并列偏好机械拆成一个词一条节点
...
9. 只有在以下情况才应把同领域事实继续拆成多条节点：
   - 它们存在不同的优先级或生命周期
   - 其中一部分被明确否定、另一部分仍然成立
   - 它们虽然同领域，但后续检索与替代应独立处理
10. 如果旧节点本身是跨领域的大泥球，而新指令只修改其中某个领域：
   - 你必须把旧节点拆解成多个新的替代节点
   - 继续保留未冲突且仍有效的领域事实
   - 不要生成新的跨领域综合节点
11. `supersede_nodes.node_id` 与 `retired_nodes.node_id` 只能引用输入里的 `active_nodes.id`
12. 同一个旧节点只能被退役一次：
   - 不能同时出现在多个 `supersede_nodes`
   - 也不能同时出现在 `retired_nodes`
13. 如果用户指令里没有任何值得进入长期画像系统的新内容，也没有任何旧节点需要退役：
   - 可以返回空的 `accepted_nodes`
   - 也可以返回空的 `retired_nodes`
   - 但必须在 `reason` 里解释为什么
14. 只返回 json，不要输出解释文字，不要输出 markdown，不要包围栏。
...
- 这仍然是同一个“水果/饮食偏好”领域
- 结果可以合并成一条新的同领域节点，例如“用户偏好食用苹果、香蕉和梨”
- 不要机械地拆成“苹果一条、香蕉一条、梨一条”
