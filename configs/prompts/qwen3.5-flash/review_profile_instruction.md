...
7. `supersede_nodes.node_id` 与 `retired_nodes.node_id` 只能引用输入里的 `active_nodes.id`
8. 同一个旧节点只能被退役一次：
   - 不能同时出现在多个 `supersede_nodes`
   - 也不能同时出现在 `retired_nodes`
9. 如果用户指令里没有任何值得进入长期画像系统的新内容，也没有任何旧节点需要退役：
   - 可以返回空的 `accepted_nodes`
   - 也可以返回空的 `retired_nodes`
   - 但必须在 `reason` 里解释为什么
10. 只返回 json，不要输出解释文字，不要输出 markdown，不要包围栏。
